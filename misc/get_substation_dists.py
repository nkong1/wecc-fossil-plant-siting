...
If there are <50 candidates for any tech in a load zone, use the average distance instead of the 10th percentile
"""
...
electricity_grouped_candidates = (
    base_path.parent / "electricity" / "inputs" / "final_candidates"
)
hydrogen_grouped_candidates = (
    base_path.parent / "hydrogen" / "inputs" / "final_candidates"
)
...
def get_connection_dists(candidates_path):
    ...
    for file_path in candidates_path.glob("*gpkg"):
        ...
        for load_zone in load_zones:
            tech_candidates_filtered = tech_candidates[
                tech_candidates["LOAD_AREA"] == load_zone
            ]
            connection_dist_col = tech_candidates_filtered["dist_to_substation_meters"]

            if len(connection_dist_col) < 50:
...
gen_dists = get_connection_dists(electricity_grouped_candidates).sort_values(
    ["load_zone", "tech"]
)
prod_dists = get_connection_dists(hydrogen_grouped_candidates).sort_values(
    ["load_zone", "tech"]
)

gen_dists.to_csv(base_path / "gen_connection_dists.csv", index=False)
prod_dists.to_csv(base_path / "prod_connection_dists.csv", index=False)
